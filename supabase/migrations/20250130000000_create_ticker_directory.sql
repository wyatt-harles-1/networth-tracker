...
DROP POLICY IF EXISTS "Ticker directory is readable by all authenticated users" ON ticker_directory;
...
DROP POLICY IF EXISTS "Users can insert validated tickers" ON ticker_directory;
...
DROP POLICY IF EXISTS "Users can update ticker directory" ON ticker_directory;
...
DROP POLICY IF EXISTS "Update logs are readable by authenticated users" ON ticker_directory_updates;
CREATE POLICY "Update logs are readable by authenticated users"
  ON ticker_directory_updates FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "System can insert update logs" ON ticker_directory_updates;
CREATE POLICY "System can insert update logs"
  ON ticker_directory_updates FOR INSERT
  TO authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS "System can update logs" ON ticker_directory_updates;
