...
CREATE OR REPLACE FUNCTION update_portfolio_value_history_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
DROP TRIGGER IF EXISTS update_portfolio_value_history_updated_at ON portfolio_value_history;
CREATE TRIGGER update_portfolio_value_history_updated_at
  BEFORE UPDATE ON portfolio_value_history
  FOR EACH ROW
  EXECUTE FUNCTION update_portfolio_value_history_updated_at();
...
DO $$
BEGIN
  RAISE NOTICE '✓ Created portfolio_value_history table';
  RAISE NOTICE '✓ Created portfolio_calculation_jobs table';
  RAISE NOTICE '✓ Created market_value_snapshots table';
END $$;
